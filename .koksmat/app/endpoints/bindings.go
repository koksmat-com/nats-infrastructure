...
func AddEndpoints(s *web.Service, jwtAuth func(http.Handler) http.Handler) {
	s.Route("/v1", func(r chi.Router) {
		r.Group(func(r chi.Router) {
			...
			r.Method(http.MethodPost, "/server/kill", nethttp.NewHandler(ServerKillPost()))
			r.Method(http.MethodPost, "/server/start", nethttp.NewHandler(ServerStartPost()))
			r.Method(http.MethodPost, "/provision/superset-ingress", nethttp.NewHandler(ProvisionSupersetIngressPost()))
